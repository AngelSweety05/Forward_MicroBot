...
async def edit(user, msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append([InlineKeyboardButton('• ᴄᴏᴍᴘʟᴇᴛᴇᴅ ​•', url='https://t.me/ComanderAK')])
   else:
...
async def restart_pending_forwads(bot, user):
    ...
    try:
       ...
       try: 
          ...
          await msg_edit(m, 
                         f"**Source chat may be a private channel / group. Use userbot (user must be member over there) or  if Make Your [Bot](t.me/{_bot['username']}) an admin over there**", 
                         retry_btn(forward_id), 
                         True)
          return await stop(client, user)
       try:
          k = await client.send_message(i.TO, "Testing")
